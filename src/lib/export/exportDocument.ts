...
MADE IN KOREA</div>
  ...
  <div class="footer">Generated by Fleety AI · ${new Date().toLocaleDateString('ko-KR')}</div>
</div></body></html>`;
}
...
<body><div class="page">
  <div class="header">
    <div style="font-size:12px;color:rgba(255,255,255,.8);margin-bottom:4px;letter-spacing:.2em">${args.seller?.company_name ?? 'Fleety'}</div>
  ...
  <div class="footer">Generated by Fleety AI · Confidential · ${new Date().toLocaleDateString('ko-KR')}</div>
</div></body></html>`;
}
...
function buildProposalDOCX(args: ProposalArgs): Document {
  ...
  const children: (Paragraph | Table)[] = [
    new Paragraph({ text: args.seller?.company_name ?? 'Fleety', heading: HeadingLevel.HEADING_2, alignment: AlignmentType.CENTER }),
  ...
  children.push(
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: `Generated by Fleety AI · Confidential · ${new Date().toLocaleDateString('ko-KR')}`,
          color: '999999',
          size: 18,
        }),
      ],
...
function buildTradeDocDOCX(args: TradeDocArgs, docType: string): Document {
  ...
  const footerPara = new Paragraph({
    children: [new TextRun({ text: `Generated by Fleety AI · ${new Date().toLocaleDateString('ko-KR')}`, size: 14, color: MUTED, font: FONT })],
    alignment: AlignmentType.CENTER,
    spacing: { before: 200 },
  });
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `Generated by Fleety AI · ${new Date().toLocaleDateString('ko-KR')}`,
                color: '999999',
                size: 18,
              }),
            ],
...
/** PI / CI / PL / NDA / SALES_CONTRACT → Fleety_[TYPE]_[YYYYMMDD].pdf */
export async function downloadTradeDocAsPDF(
  ...
  triggerDownload(blob, `Fleety_${docType}_${todayCompact()}.pdf`);
}

/** COMPLIANCE → Fleety_COMPLIANCE_[YYYYMMDD].pdf */
export async function downloadComplianceAsPDF(
  ...
  triggerDownload(blob, `Fleety_COMPLIANCE_${todayCompact()}.pdf`);
}

/** PI / CI / PL / NDA / SALES_CONTRACT → Fleety_[TYPE]_[YYYYMMDD].docx */
export async function downloadTradeDocAsWord(
  ...
  saveAs(blob, `Fleety_${docType}_${todayCompact()}.docx`);
}

/** COMPLIANCE → Fleety_COMPLIANCE_[YYYYMMDD].docx */
export async function downloadComplianceAsWord(
  ...
  saveAs(blob, `Fleety_COMPLIANCE_${todayCompact()}.docx`);
}

/** PROPOSAL → Fleety_PROPOSAL_[YYYYMMDD].pdf */
export async function downloadProposalAsPDF(
  args: Record<string, unknown>,
): Promise<void> {
  const blob = await htmlToBlob(buildProposalHTML(args as unknown as ProposalArgs));
  triggerDownload(blob, `Fleety_PROPOSAL_${todayCompact()}.pdf`);
}

/** PROPOSAL → Fleety_PROPOSAL_[YYYYMMDD].docx */
export async function downloadProposalAsWord(
  ...
  saveAs(blob, `Fleety_PROPOSAL_${todayCompact()}.docx`);
}
